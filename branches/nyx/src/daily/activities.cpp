...
void funds_and_trouble(char &clearformess)
{
   ...
   //PROSTITUTION
   //XXX Should it depend on human rights/civil rights status?
   for(int p=prostitutes.size()-1;p>=0;p--)
   {
      ...
      if (prostitutes[p]->age >=18) // Adult prostitute assumes self
      {
          if(!LCSrandom(3) &&
      ...
      else // Minor of age prostitute - gets 1-3/40 juice penalty, pimp take 1-5/-200 juice penalty anytime
           // XXX Should be worse or different?
      {
          if(!LCSrandom(2) &&
             !(prostitutes[p]->skill_check(SKILL_STREETSENSE,DIFFICULTY_AVERAGE)))
          {
             addjuice(*prostitutes[p],-!LCSrandom(3), -40);
          }
      ...
      if(!LCSrandom(50)) // Police sting?
      {
         if(!(prostitutes[p]->skill_check(SKILL_STREETSENSE,DIFFICULTY_AVERAGE))) // Street sense to avoid
         {
            ...
            else // Minor of age prostitute - pimp gets trouble too
                 // XXX Severity of law should depend on human rights status? Illegal immigrant and c+ civil rights?
            {
				int pimp = getpoolcreature(prostitutes[p]->hireid);
                set_color(COLOR_RED,COLOR_BLACK,1);
                addstr("Minor ");
				...
				criminalize(*pool[pimp],LAWFLAG_EXTORTION);
				criminalize(*pool[pimp],LAWFLAG_KIDNAPPING);
				//criminalize(*pool[pimp],LAWFLAG_TERRORISM);
            }

   ...
   while(true)
   {
      ...
      case PAGE_LEADERS:
      {
         ...
         else
         {
            ...
            if(housemake[5]+MIN(housemake[0],housemake[4])>=HOUSEMAJORITY) align=Alignment::STALINIST; // Stalinists have a majority (perhaps with help from extremists on both sides)
            else if(housemake[0]>=HOUSEMAJORITY) align=Alignment::ARCH_CONSERVATIVE; // Arch-Conservatives have a majority
            else if(housemake[4]>=HOUSEMAJORITY) align=Alignment::ELITE_LIBERAL; // Elite Liberals have a majority
            else if(housemake[0]+housemake[1]>=HOUSEMAJORITY) align=Alignment::CONSERVATIVE; // Conservatives plus Arch-Conservatives have a majority
            else if(housemake[3]+housemake[4]>=HOUSEMAJORITY) align=Alignment::LIBERAL; // Liberals plus Elite Liberals have a majority
            else align=Alignment::MODERATE; // nobody has a majority
            set_alignment_color(align,true);
            mvaddstr(10,0,"House: ");
            ...
            if(senatemake[5]+MIN(senatemake[0],senatemake[4])>=SENATEMAJORITY) align=Alignment::STALINIST; // Stalinists have a majority (perhaps with help from extremists on both sides)
            else if(senatemake[0]>=SENATEMAJORITY) align=Alignment::ARCH_CONSERVATIVE; // Arch-Conservatives have a majority
            else if(senatemake[4]>=SENATEMAJORITY) align=Alignment::ELITE_LIBERAL; // Elite Liberals have a majority
            else if(senatemake[0]+senatemake[1]>=SENATEMAJORITY) align=Alignment::CONSERVATIVE; // Conservatives plus Arch-Conservatives have a majority
            else if(senatemake[3]+senatemake[4]>=SENATEMAJORITY) align=Alignment::LIBERAL; // Liberals plus Elite Liberals have a majority
            else align=Alignment::MODERATE; // nobody has a majority
            set_alignment_color(align,true);
         ...
         else
         {
            ...
            if(courtmake[5]+MIN(courtmake[0],courtmake[4])>=COURTMAJORITY) align=Alignment::STALINIST; // Stalinists have a majority (perhaps with help from extremists on both sides)
            else if(courtmake[0]>=COURTMAJORITY) align=Alignment::ARCH_CONSERVATIVE; // Arch-Conservatives have a majority
            else if(courtmake[4]>=COURTMAJORITY) align=Alignment::ELITE_LIBERAL; // Elite Liberals have a majority
            else if(courtmake[0]+courtmake[1]>=COURTMAJORITY) align=Alignment::CONSERVATIVE; // Conservatives plus Arch-Conservatives have a majority
            else if(courtmake[3]+courtmake[4]>=COURTMAJORITY) align=Alignment::LIBERAL; // Liberals plus Elite Liberals have a majority
            else align=Alignment::MODERATE; // nobody has a majority
            set_alignment_color(align,true);
         }
         ...
         for(int l=0;l<LAWNUM;l++)
         {
            if(won==-1||won==-2)
               ...
               set_alignment_color(Alignment::ELITE_LIBERAL,true);
            ...
            addstr(getlaw(l));
            mvaddchar(14+l/3, l%3*26 + 1 - to_index(law[l]), 'O');
         }
         break;
      }
            ...
            switch(l)
            {
               case LAW_WOMEN:
                  if(won==-2)
                    ...
                    addstr("Women are considered property, and rape has been legalized.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                     addstr("Women are second-class citizens.");
                  else if(law[l] == Alignment::CONSERVATIVE)
                    addstr("Non-discrimination laws do not apply to gender.");
                  else if(law[l] == Alignment::MODERATE)
                    addstr("Women are nominally equal under law, but this is not enforced.");
                  else if(law[l] == Alignment::LIBERAL)
               ...
               case LAW_CIVILRIGHTS:
                  if (won==-2)
                    ...
                    addstr("Slavery has been reintroduced, along with an apartheid system.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                    ...
                    addstr("Pervasive racial inequality exists, although overt discrimination is illegal.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("Affirmative action is in place to counteract racial discrimination.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
               ...
               case LAW_DRUGS:
                  if (won==-2)
                    ...
                    addstr("Marijuana is regulated and taxed, but harder drugs are illegal.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
                    addstr("Recreational drugs all are regulated and taxed like alcohol and tobacco.");
                  else
               ...
               case LAW_IMMIGRATION:
                  if (won==-2)
                    ...
                    addstr("Private border militiamen shoot suspected foreigners on sight.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                     addstr("Immigration is illegal, and noncitizens are shipped to Mexico at gunpoint.");
                  else if(law[l] == Alignment::CONSERVATIVE)
                    addstr("The National Guard has been deployed to the borders to slow immigration.");
                  else if(law[l] == Alignment::MODERATE)
                    addstr("Great expense is taken to slow immigration, without success.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("The government works to accommodate potential immigrants but deports criminals.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
                    addstr("Immigration is unregulated, and new immigrants are welcomed warmly.");
                  else
               ...
               case LAW_ELECTIONS:
                  if(won==-2)
                    ...
                    addstr("Instead of elections, political offices are auctioned off to the highest bidder.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
               ...
               case LAW_MILITARY:
                  if(won==-2)
                    ...
                    addstr("The massive military kills dissenters at home and conquers poor nations abroad.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                    addstr("Out-of-control military spending funds several ongoing wars around the world.");
                  else if(law[l] == Alignment::CONSERVATIVE)
                    addstr("Massive investment is put into the military, which always seems to be at war.");
                  else if(law[l] == Alignment::MODERATE)
                    addstr("Military spending is growing each year to fund overseas military adventures.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("The military is not a major priority, and mostly does peacekeeping missions.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
                    addstr("The military has been weakened significantly, as there is little need for it.");
                  else
                    addstr("The military has been abolished, and the entire world is at peace.");
                  break;
               case LAW_TORTURE:
                  if(won==-2)
                    addstr("The Internal Affairs Commissariat constantly invents new methods of torture.");
                  else if(won==-1)
                    addstr("The new Inquisition tortures heretics, blasphemers, and non-Christians to death.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                    ...
                    addstr("Torture allegations still occasionally crop up, despite an official ban.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("The government strongly enforces a ban on torture.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
                    addstr("The nation is a respected international leader on Moral Interrogation Practices.");
                  else
               ...
               case LAW_PRISONS:
                  if(won==-2)
                    ...
                    addstr("Prisoners must fight to the death in corporate-sponsored gladiatorial bouts.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                    ...
                    addstr("Prisoners receive basic rights and services, but reports of abuse are common.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("The prisons are regulated to protect prisoners' rights and safety.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
               ...
               case LAW_TAX:
                  if(won==-2)
                    ...
                    addstr("A flat tax is in effect, and there is no capital gains or inheritance tax.");
                  else if(law[l] == Alignment::MODERATE)
                    addstr("Taxes are moderate, but the code is full of loopholes.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("The wealthy are heavily taxed under a progressive taxation system.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
               ...
               case LAW_ABORTION:
                  ...
                  else if(law[l] == Alignment::CONSERVATIVE)addstr("Abortion is prohibited except in cases of rape, incest, or health of the mother.");
                  else if(law[l] == Alignment::MODERATE)addstr("Abortion is limited to the first trimester, and is very expensive.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Abortion is legal, but taxpayer funding of abortion is prohibited.");
               ...
               case LAW_ANIMALRESEARCH:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Animal testing is self-regulated by the scientific community.");
                  else if(law[l] == Alignment::MODERATE)addstr("Animal research is regulated with a system of licenses and certificates.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Animal research is strictly regulated by purpose and suffering caused.");
               ...
               case LAW_POLICEBEHAVIOR:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Even the worst police misconduct only earns slap-on-the-wrist punishments.");
                  else if(law[l] == Alignment::MODERATE)addstr("Law enforcement is regulated to prevent extreme misconduct.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Law enforcement has heavy oversight and freedom-of-information requirements.");
               ...
               case LAW_PRIVACY:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Privacy laws are full of loopholes and security backdoors are in everything.");
                  else if(law[l] == Alignment::MODERATE)addstr("Basic safeguards for medical and financial privacy are in place but ineffective.");
                  else if(law[l] == Alignment::LIBERAL)addstr("All areas of privacy are protected with strong, effective safeguards.");
               ...
               case LAW_DEATHPENALTY:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("The death penalty is actively enforced in many states.");
                  else if(law[l] == Alignment::MODERATE)addstr("The death penalty is in effect but under scrutiny.");
                  else if(law[l] == Alignment::LIBERAL)addstr("The death penalty is only permitted in extreme cases.");
               ...
               case LAW_NUCLEARPOWER:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Nuclear power is a preferred energy source and the industry self-regulates.");
                  else if(law[l] == Alignment::MODERATE)addstr("Nuclear power is often an energy source and only moderately regulated.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Nuclear power is intensely regulated and rarely used anymore.");
               ...
               case LAW_POLLUTION:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Industry voluntarily regulates pollution.");
                  else if(law[l] == Alignment::MODERATE)addstr("Industry is subject to moderate pollution regulations.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Industry is subject to strict pollution regulations.");
               ...
               case LAW_LABOR:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Working conditions are deplorable and there is no minimum wage.");
                  else if(law[l] == Alignment::MODERATE)addstr("Workers are underpaid, have lousy benefits, and get fired if they complain.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Workers are fairly compensated, have good benefits, and are difficult to fire.");
               ...
               case LAW_GAY:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Homosexuals are not tolerated.");
                  else if(law[l] == Alignment::MODERATE)addstr("Homosexuals are grudgingly tolerated but have few equal rights.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Homosexuals have many rights shared by heterosexuals.");
               ...
               case LAW_CORPORATE:
                  ...
                  else if(law[l]==Alignment::CONSERVATIVE)addstr("Corporate culture is corrupt and there is a great disparity in wages.");
                  else if(law[l] == Alignment::MODERATE)addstr("Corporations are moderately regulated, although wages are still unfair.");
                  else if(law[l] == Alignment::LIBERAL)addstr("Corporations are stiffly regulated, and executive compensation is reasonable.");
               ...
               case LAW_FREESPEECH:
                  if (won==-2)
                    ...
                    addstr("Even *THINKING* about saying something unacceptable is a capital crime.");
                  else if(law[l]==Alignment::ARCH_CONSERVATIVE)
                    addstr("Armored squads are tasked with suppressing unacceptable speech.");
                  else if(law[l]==Alignment::CONSERVATIVE)
                    addstr("People who express unpopular opinions are often harassed and mistreated.");
                  else if(law[l] == Alignment::MODERATE)
                    addstr("Free speech is legal, with minor exceptions, and is usually tolerated.");
                  else if(law[l] == Alignment::LIBERAL)
                    addstr("Free speech is legally protected and publicly encouraged.");
                  else if(won!=1||wincondition!=WINCONDITION_ELITE)
                    addstr("Free speech is strongly protected and universally supported.");
                  else
                    addstr("Free speech is sacrosanct and diverse points of view are celebrated.");
                  break;
               case LAW_FLAGBURNING:
                  if (won==-2)
                    addstr("Flags of the old American regime are burnt primarily as fuel.");
                  else if (won == -1)
                    ...
                    addstr("Burning the flag is a felony and vigorously prosecuted.");
                  else if (law[l] == Alignment::MODERATE)
                    addstr("Flag-burning is a misdemeanor, but not a serious crime.");
                  else if (law[l] == Alignment::LIBERAL)
                    addstr("Flag-burning is technically legal but stigmatized as unpatriotic.");
                  else if (won!=1||wincondition!=WINCONDITION_ELITE)
                    addstr("The right of flag-burning is upheld even by its critics.");
                  else
               ...
               case LAW_GUNCONTROL:
                  if (won == -2)
                    ...
                    addstr("It is illegal to buy or sell a gun, or carry one in public.");//XXX: Should guns be legal in private, too? -- LK
                  else
                    addstr("All gun manufacturers have been shut down and all existing guns destroyed."); //They are illegal in private under Elite Liberal victory conditions - yetisyny
                  break;
            }
         }
         break;
